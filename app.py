...
from models import posts, subscribers,warden,hod,lib_books,book_issue,prev_papers,sessions
...
def getdata():
    ...
    if intentName == "posts":                       # If Query is for post search in posts table
        ...
        result=""
        r=""
        send_id=db.session.query(sessions).filter(sessions.sessionsID==sess_ID).all()[0].senderID
        print(str(send_id))
